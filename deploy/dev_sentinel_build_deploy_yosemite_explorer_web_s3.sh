...
echo "${red}Do you want to deploy YosemiteExplorerWeb DevelopmentServer for Testnet Sentinel?${reset}"
...
echo "${red}[YosemiteExplorerWeb]${green}[DevelopmentServer-Testnet-Sentinel]${reset} starting s3 bucket sync"

aws s3 sync ${BASEDIR}/../dist/ s3://testnet-sentinel-explorer.yosemitelabs.org/ --delete

echo "${red}[YosemiteExplorerWeb]${green}[DevelopmentServer-Testnet-Sentinel]${reset} s3 bucket sync finished"
