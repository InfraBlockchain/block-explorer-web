#!/bin/bash
# chmod +x dev_build_deploy_infrablockchain_explorer_web_s3.sh
...
echo "${red}Do you want to deploy InfraBlockchain ExplorerWeb DevelopmentServer?${reset}"
...
echo "${red}[InfraBlockchainExplorerWeb]${green}[DevelopmentServer]${reset} starting s3 bucket sync"

aws s3 sync ${BASEDIR}/../dist/ s3://testnet-explorer.yosemitelabs.org/ --delete

echo "${red}[InfraBlockchainExplorerWeb]${green}[DevelopmentServer]${reset} s3 bucket sync finished"
