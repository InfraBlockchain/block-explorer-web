#!/bin/bash
# chmod +x dev_sentinel_build_deploy_yosemite_explorer_web_s3.sh
...
echo "${red}Do you want to deploy YosemiteExplorerWeb Production Server for YOSEMITE Main-Net?${reset}"
...
echo "${red}[MainNet][YosemiteExplorerWeb]${green}[ProductionServer-YOSEMITE-MainNet]${reset} starting s3 bucket sync"

aws s3 sync ${BASEDIR}/../dist/ s3://mainnet.yosemite.ycard.io/ --delete

echo "${red}[MainNet][YosemiteExplorerWeb]${green}[ProductionServer-YOSEMITE-MainNet]${reset} s3 bucket sync finished"
